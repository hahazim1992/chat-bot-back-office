...
export class ChatbitAddEditComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      if (
        ['txt', 'md', 'pdf'].includes(fileExtension || '') &&
        file.size <= 10 * 1024 * 1024
      ) {
        this.newFiles.push(file);
      } else {
        alert(
          'Invalid file format or file is too large. Please upload .txt, .md, or .pdf files under 10MB.'
        );
      }
    }
  }
  ...
  viewFile(fileName: string): void {
    if (
      this.isEditMode &&
      !this.newFiles.some((file) => file.name === fileName)
    ) {
  ...
  loadChatbotData(chatbotName: string): void {
    this.chatbotService
      .getSingleChatbot(chatbotName)
      .subscribe((singleChatbot: any) => {
